    ...
    <main>
      ...
      <section className="service-faq">
        ...
        <div className="faq-list">
          <details>
            <summary>View the full cleaning checklist <span>+</span></summary>
            <div style={{ paddingTop: "1rem" }}><ul className="inclusion-list">{checklist.map((item) => <li key={item}><span>✓</span>{item}</li>)}</ul></div>
          </details>
          <details>
            <summary>View starting prices — GST included <span>+</span></summary>
            <div style={{ paddingTop: "1rem" }}>
              <ul className="inclusion-list">{prices.map(([property, price]) => <li key={property}><span>✓</span><strong>{property}</strong>&nbsp; — {price}</li>)}</ul>
              <p className="service-note">Starting prices assume a standard vacant property in reasonable condition. Heavy soiling, mould, rubbish removal, extensive wall washing or unusual buildup may cost extra.</p>
            </div>
          </details>
          <details>
            <summary>View carpet, window & oven add-ons <span>+</span></summary>
            <div style={{ paddingTop: "1rem" }}><ul className="inclusion-list">{addons.map(([service, price]) => <li key={service}><span>+</span><strong>{service}</strong>&nbsp; — {price}</li>)}</ul></div>
          </details>
          <details>
            <summary>How does the bond-back cleaning guarantee work? <span>+</span></summary>
            <p>If your property manager identifies a reasonable cleaning issue in an area included in your Syme Cleaning scope, contact us promptly and we will return to re-clean that area free of charge. The guarantee applies to our included cleaning work only and cannot guarantee the return of a bond for damage, rent, rubbish, maintenance or other tenancy matters.</p>
          </details>
          <details>
            <summary>Do I need to book carpets and windows too? <span>+</span></summary>
            <p>No. The base end of tenancy clean can be booked on its own. Carpet cleaning, window cleaning, oven/rangehood cleaning and garage cleaning can be added where required.</p>
          </details>
        </div>
      </section>
      ...
      <section className="quote-section service-quote" id="end-of-tenancy-quote">
        <div className="quote-intro">
          <span className="kicker kicker-light">Free quote</span>
          <h2>Tell us about the property.</h2>
          <p>Send the basics below. We will use the property size, condition and add-ons to give you the next step.</p>
          <ul className="trust-list"><li><span>✓</span> Bond-back cleaning guarantee on included work</li><li><span>✓</span> 5-star Google reviews</li><li><span>✓</span> Fully insured</li><li><span>✓</span> Local, owner-led service</li></ul>
        </div>
        <EndOfTenancyForm />
      </section>
